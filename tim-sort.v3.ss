...
(define (tim-sort-range! lt? v lo hi)
  ...
  (define (ensure-capacity! min-gallop)
    (if (fx< (vector-length tmp-array) min-gallop)
        (let ([len (tmp-array-length min-gallop)])
          ; (printf "[95] old-length: ~a\n" (vector-length tmp-array))
          ; (printf "[96] new-length: ~a\n" len)
  ...
  (define (count-run-and-make-ascending lo hi)
    ...
    (let ([run-hi (fx+ lo 1)])
      (define-syntax (while-loop x)
        (syntax-case x ()
          [(k maybe-not maybe-reverse ...)
           (with-implicit (k run-hi*)
      ...
      (cond
        ...
        [(lt? (v-get run-hi) (v-get lo))
         (while-loop and (reverse-range lo (fx- run-hi* 1)))]
        ;; ascending
        [else (while-loop not)])))
  ...
  (define-syntax 1st (syntax-rules () [(_ x y) x]))
  (define-syntax 2nd (syntax-rules () [(_ x y) y]))

  (define-syntax gallop
    (syntax-rules ()
      [(k key a base len hint 1st 2nd ofs+ ofs- swap)
       (let-syntax ([get (syntax-rules () [(_ i) (vector-ref a i)])])
         (let ([base+hint (fx+ base hint)])
           (define-syntax gallop-branch
             (syntax-rules ()
               [(_ first ofs+ not)
                (let ([max-ofs (first (fx- len hint) (fx+ hint 1))])
                  (let while ([last-ofs 0] [ofs 1])
                    (cond
                      [(and (fx< ofs max-ofs)
                            (not (swap lt? key (get (ofs+ base+hint ofs)))))
                       (while ofs
                         (let ([ofs (fx+ (fxsll ofs 1) 1)])
                           (if (fx<= ofs 0) max-ofs ofs)))]
                      [else
                       (let ([ofs (if (fx> ofs max-ofs) max-ofs ofs)])
                         (do-assert
                           (and (fx<= -1 last-ofs)
                                (fx< last-ofs ofs)
                                (fx<= ofs len)))
                         (let ([last-ofs (first (fx+ last-ofs hint) (fx- hint ofs))]
                               [ofs (first (fx+ ofs hint) (fx- hint last-ofs))])
                           (let while ([last-ofs (fx+ last-ofs 1)] [ofs ofs])
                             (cond
                               [(fx< last-ofs ofs)
                                (let ([m (fx+ last-ofs (fxsrl (fx- ofs last-ofs) 1))])
                                  (swap if
                                        (swap lt? key (get (fx+ base m)))
                                        (while last-ofs m)
                                        (while (fx+ m 1) ofs)))]
                               [else
                                (do-assert (fx= last-ofs ofs))
                                ofs]))))])))]))
           (if (swap lt? key (get base+hint))
               (gallop-branch 1st ofs+ and)
               (gallop-branch 2nd ofs- not))))]))

  (define (gallop-left key a base len hint)
    (do-assert (and (fx> len 0) (fx>= hint 0) (fx< hint len)))
    (gallop key a base len hint 1st 2nd fx+ fx- swap))

  (define (gallop-right key a base len hint)
    (do-assert (and (fx> len 0) (fx>= hint 0) (fx< hint len)))
    (gallop key a base len hint 2nd 1st fx- fx+ keep))
...
(time (tim-sort < ...))
    7 collections
    5.875000000s elapsed cpu time, including 0.281250000s collecting
    6.102008100s elapsed real time, including 0.328489300s collecting
    155559376 bytes allocated, including 220037040 bytes reclaimed
comparations: 220247255, ordered: #t
(time (tim-sort < ...))
    no collections
    0.078125000s elapsed cpu time
    0.075848900s elapsed real time
    2976 bytes allocated
comparations: 9999999, ordered: #t
(time (tim-sort >= ...))
    no collections
    0.125000000s elapsed cpu time
    0.128943100s elapsed real time
    2976 bytes allocated
comparations: 9999999, ordered: #t
(time (tim-sort <= ...))
    no collections
    0.109375000s elapsed cpu time
    0.110580700s elapsed real time
    2976 bytes allocated
comparations: 9999999, ordered: #t
(time (bench-tim-sort!))
    16 collections
    13.921875000s elapsed cpu time, including 1.265625000s collecting
    14.377265800s elapsed real time, including 1.355234000s collecting
    631185664 bytes allocated, including 419896576 bytes reclaimed

(time (vector-sort < ...))
    no collections
    3.468750000s elapsed cpu time
    3.545327100s elapsed real time
    80000064 bytes allocated
comparations: 240976582, ordered: #t
(time (vector-sort < ...))
    no collections
    0.218750000s elapsed cpu time
    0.214416600s elapsed real time
    80000064 bytes allocated
comparations: 9999999, ordered: #t
(time (vector-sort >= ...))
    no collections
    1.750000000s elapsed cpu time
    1.819392600s elapsed real time
    80000064 bytes allocated
comparations: 124434623, ordered: #t
(time (vector-sort <= ...))
    no collections
    1.765625000s elapsed cpu time
    1.848428800s elapsed real time
    80000064 bytes allocated
comparations: 124434623, ordered: #t
(time (bench-vector-sort!))
    4 collections
    17.703125000s elapsed cpu time, including 1.515625000s collecting
    18.352614800s elapsed real time, including 1.591367200s collecting
    960048192 bytes allocated, including 720071040 bytes reclaimed
