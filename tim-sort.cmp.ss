...
(define (tim-sort-range! cmp v lo hi)
  (define MIN_MERGE 32)
  (define MIN_GALLOP 7)
  ...
  (define (count-run-and-make-ascending lo hi)
    ...
    (let ([run-hi (fx+ lo 1)])
      (cond
        [(fx= run-hi hi) 1]
        [(fx< (cmp (v-get run-hi) (v-get lo)) 0)
         (let while ([run-hi (fx+ run-hi 1)])
           (if (and (fx< run-hi hi)
                    (fx< (cmp (v-get run-hi) (v-get (fx- run-hi 1))) 0))
               (while (fx+ run-hi 1))
               (begin
  ...
  (define (merge-collapse)
    (define (helper n)
      (cond
        [(and (fx> n 0) (fx<= (rl-get (fx- n 1))
                          (fx+ (rl-get n) (rl-get (fx+ n 1)))))
         (if (fx< (rl-get (fx- n 1)) (rl-get (fx+ n 1))) (fx- n 1) n)]
  ...
  (define-syntax gallop
    (syntax-rules ()
      [(_ max-ofs* cmp-with-ofs compute-with-hint last-while-loop)
       (let ([max-ofs max-ofs*])
         (let while ([last-ofs 0][ofs 1])
           (cond
             [(and (fx< ofs max-ofs) (cmp-with-ofs ofs))
        ...
        (cond
          [(fx< last-ofs ofs)
           (let ([m (fx+ last-ofs (fxsrl (fx- ofs last-ofs) 1))])
             (if (fx> (cmp key (get (fx+ base m))) 0)
                 (while (fx+ m 1) ofs)
                 (while last-ofs m)))]
          [else (do-assert (fx= last-ofs ofs)) ofs])))
    (do-assert (and (fx> len 0) (fx>= hint 0) (fx< hint len)))

    (if (fx> (cmp key (get (fx+ base hint))) 0)
        (gallop (fx- len hint)
                (lambda (ofs) (fx> (cmp key (get (fx+ base hint ofs))) 0))
                (compute-with-hint1 hint)
                last-while)
        (gallop (fx+ hint 1)
                (lambda (ofs) (fx<= (cmp key (get (fx- (fx+ base hint) ofs))) 0))
  ...
  (define (merge-lo base1 len1 base2 len2)
    (define (do-while1 cnt1 cnt2 cur1 cur2 dest len1 len2 mgp)
      ...
      (cond
        [(fx< (cmp (v-get cur2) (t-get cur1)) 0)
        ...
        [else
         ...
         (let ([dest (fx+ dest 1)]
               ...
               [len1 (fx- len1 1)])
           (cond
             [(fx= len1 1) (break cur1 cur2 dest len1 len2 mgp)]
             [else
  ...
  (define (merge-hi base1 len1 base2 len2)
      (define (do-while1 cnt1 cnt2 cur1 cur2 dest len1 len2 mgp)
        ...
        (cond
          [(fx< (cmp (t-get cur2) (v-get cur1)) 0)
           (v-set! dest (v-get cur1))
           (let ([dest (fx- dest 1)]
                 [cur1 (fx- cur1 1)]
                 [len1 (fx- len1 1)])
             (cond
               [(fx= len1 0) (break cur1 cur2 dest len1 len2 mgp)]
               [else
                (let ([cnt1 (fx+ cnt1 1)][cnt2 0])
                  (if (fx< (fxlogor cnt1 cnt2) mgp)
                      (do-while1 cnt1 cnt2 cur1 cur2 dest len1 len2 mgp)
          ...
          [else
           ...
           (let ([dest (fx- dest 1)]
                 ...
                 [len2 (fx- len2 1)])
             (cond
               [(fx= len2 1) (break cur1 cur2 dest len1 len2 mgp)]
               [else
                (let ([cnt2 (fx+ cnt2 1)][cnt1 0])
                  (if (fx< (fxlogor cnt1 cnt2) mgp)
                      (do-while1 cnt1 cnt2 cur1 cur2 dest len1 len2 mgp)
...
(define (tim-sort! cmp v)
  (tim-sort-range! cmp v 0 (vector-length v)))

(define (tim-sort cmp v)
  (tim-sort! (vector-copy v)))
